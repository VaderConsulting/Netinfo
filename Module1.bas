...
Public Function IsNetConnectViaLAN() As Boolean
    ...
    'pass an empty variable into which the API will return the flags associated with the connection
...
Public Function IsNetConnectOnline() As Boolean
    'no flags needed here - the API returns True if there is a connection of any type
